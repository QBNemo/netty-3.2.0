...
public class DefaultChannelPipeline implements ChannelPipeline {
     ...
     */
    // 注意head要非null, 否则报NullPointException
    @Override
    public String toString() {
        StringBuilder buf = new StringBuilder();
        buf.append(getClass().getSimpleName());
        buf.append('{');
        buf.append("@" + Integer.toHexString(this.hashCode()) + " ");  // add
        DefaultChannelHandlerContext ctx = head;
        for (;;) {
            buf.append('(');
            buf.append(ctx.getName());
            buf.append(" = ");
            buf.append(ctx.getHandler().getClass().getName() + "@" + Integer.toHexString(ctx.getHandler().hashCode())); // add hashCode
            buf.append(')');
            ctx = ctx.next;
            if (ctx == null) {
                break;
    ...
    public void sendUpstream(ChannelEvent e) {
    	...
    	// 从head开始
        // 如果head为null, this.toString()会报NullPointException
        // NioDatagramChannelTest 
    	log.debug(this.toString() + " >> " +e.toString());
        sendUpstream(head, e);
    }

    void sendUpstream(DefaultChannelHandlerContext ctx, ChannelEvent e) {
    	// 指定handler
    	log.debug(ctx.toString() + " >> " +e.toString());
        try {
    ...
    private final class DefaultChannelHandlerContext implements ChannelHandlerContext {
        ...
        public void sendUpstream(ChannelEvent e) {
            ...
            if (next != null) {
            	log.debug(DefaultChannelPipeline.this.toString() + " -> " + next.toString());
                DefaultChannelPipeline.this.sendUpstream(next, e);
            }
        }

		//@Override
		// NioOioSocketFixedLengthEchoTest, override toString cause junit test fail
		public String toString_() {
	        StringBuilder buf = new StringBuilder(64);
	        buf.append("[");
	        buf.append("context: " + this.getClass().getSimpleName() + "@" + Integer.toHexString(this.hashCode()));
	        buf.append(", name: " + name);
	        buf.append(", handler: " + handler.toString());
	        if(prev!=null) {
	        	buf.append(", prev: " + prev.toString());
	        } else {
	        	buf.append(", prev: null");
	        }
	        if(next!=null) {
	        	buf.append(", next: " + next.toString());
	        } else {
	        	buf.append(", next: null");
	        }
	        buf.append(']');
	        return buf.toString(); 
		}
